...
let RecorderApp = function RecorderApp(
    ...
    let GLOBALS = {
        ...
        setLoResInPoint: function (v) {
            this.loResInPoint = v;
            instance.fullResInPoint =
                typeof v === "number" ? binarySearch(instance.audEng.codeChannel, v) : undefined;
        },
        setLoResOutPoint: function (v) {
            this.loResOutPoint = v;
            instance.fullResOutPoint =
                typeof v === "number" ? binarySearch(instance.audEng.codeChannel, v) : undefined;
        },
    ...
    this.saveEngine = function () {
        if (this.currentSave) {
            ...
            do {
                ...
                if (!progress.done) {
                    if (this.saveModeUpdateCallback) {
                        this.saveModeUpdateCallback(progress.value);
                    }
                }
    ...
    recordState.handleWaveformClick = function recordStateHandleWaveformClick(code) {
        if (code >= GLOBALS.loResInPoint) {
            // outpoint must be after in point!
    ...
    saveState.execute = function saveStateExecute() {
        let saveCompleteCallback = function saveCompleteCallback(WAVFileBlob) {
            ...
            if (this.recordingsListChangedCallback) {
    ...
    saveState.enter = function saveStateEnter() {
        if (this.enteringSaveModeCallback) {
            this.enteringSaveModeCallback();
        }
    }.bind(this);
    ...
    saveState.exit = function saveStateExit() {
        ...
        GLOBALS.setLoResOutPoint(undefined);
        if (this.exitingSaveModeCallback) {
            this.exitingSaveModeCallback();
        }
    }.bind(this);

    this.toggleOptionalAudioConstraint = function recorderToggleOptionalAudioConstraint(
        constraintName
    ) {
    ...
    this.init = function RecorderAppInit() {
        ...
        this.audEng = new AudioEngine(GLOBALS, audioOptions, {
        ...
        if (this.MouseStatus) {
            this.mouse = new this.MouseStatus(this.canvas);
        }
    ...
    this.vmRecordings = function vmRecordings() {
        ...
        recordings.forEach(function recordingsForEach(recording) {
            let recObj = {
                ...
                // TODO refactor humaneDate to use two UTC datestamps
                date: humaneDate(recording.UCTTimestamp, Date.now()),
    ...
    this.getMemory = function getMemory() {
        ...
        out.buffers = formatBytes(
            this.audEng.audioData.length * this.audEng.scriptProcessorBufferLength
        );
        return out;
    }.bind(this);
    ...
    this.changeLengthOrQuality = function changeLengthOrQuality(bufferLength, audioOptions) {
        if (GLOBALS.state === "save") {
            ...
            this.audEng = new AudioEngine(GLOBALS, this.audioOptions, {
                loResWaveformParams: this.loResWaveformParams,
                optionalMediaConstraints: this.optionalMediaConstraints,
                scriptProcessorBufferLength: this.scriptProcessorBufferLength
            });
